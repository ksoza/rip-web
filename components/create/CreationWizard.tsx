...
export function CreationWizard({ user, selectedMedia, onClose, onOpenEditor }: Props) {
  ...
  // ── Generate video for scenes (NEW: Phase 2) ─────────────────
  async function generateVideos() {
    setVideoGenerating(true);
    setVideoProgress(0);
    setVideoStage('Starting video generation...');
    const totalScenes = scenes.length;
    let completed = 0;

    for (let i = 0; i < totalScenes; i++) {
      const scene = scenes[i];
      setVideoStage(`🎬 Animating Scene ${i + 1}/${totalScenes}: ${scene.description.slice(0, 40)}...`);
      ...
      try {
        ...
        console.error(`Video gen error scene ${i + 1}:`, err);
      }

      completed++;
      setVideoProgress(Math.round((completed / totalScenes) * 100));
    }
