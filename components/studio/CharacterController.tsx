...
const POSES: { id: PosePreset; icon: string; label: string }[] = [
  ...
  { id: 'pointing',       icon: '👉', label: 'Pointing' },
  { id: 'crossed-arms',   icon: '💪', label: 'Arms Crossed' },
  { id: 'hands-on-hips',  icon: '🦸', label: 'Hands on Hips' },
  { id: 'waving',         icon: '👋', label: 'Waving' },
...
const ANGLES: { id: CameraAngle; icon: string; label: string }[] = [
  ...
  { id: 'close-up',       icon: '🔍', label: 'Close-up' },
  { id: 'full-body',      icon: '📏', label: 'Full Body' },
  { id: 'medium-shot',    icon: '📐', label: 'Medium Shot' },
];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-display text-2xl text-white tracking-wide">🎮 CHARACTER <span className="text-rip">CONTROLLER</span></h2>
        ...
        <>
          ...
          >
            {loading
              ? '⏳ Generating...'
              : batchMode
                ? `🎨 Generate ${selectedExpressions.length} Expression${selectedExpressions.length !== 1 ? 's' : ''}`
                : '🎨 Generate Pose'
            }
          </button>
                  ...
                  <div key={g.id} className="relative rounded-lg overflow-hidden border border-border bg-bg3 group">
                    ...
                    {g.imageUrl && (
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-2">
                        <button className="px-2 py-1 rounded bg-rip text-[9px] font-bold text-white">Use as Face</button>
                      </div>
                    )}
                  </div>
                ))}
