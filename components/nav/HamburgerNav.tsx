...
// SPLIT ARCHITECTURE: Button renders in header, Panel renders via portal
// to avoid backdrop-filter stacking-context clipping the fixed overlay.
...
// ── Shared open state via context so Button + Panel stay in sync ───
interface NavCtx { open: boolean; toggle: () => void; close: () => void; }
const NavContext = createContext<NavCtx>({ open: false, toggle: () => {}, close: () => {} });

/**
 * HamburgerNav — renders ONLY the trigger button inline.
 * The slide-out panel is portalled to document.body so it escapes
 * any parent stacking-context (header backdrop-blur, etc.).
 */
export function HamburgerNav({ currentPage, onNavigate, user, profile, onSignIn }: Props) {
  ...
  return (
    <>
      {/* ── Hamburger Button (renders inline in header) ─────────── */}
      <button
        onClick={toggle}
      ...
      {/* ── Slide-out Panel + Backdrop (portalled to body) ──────── */}
      {mounted && createPortal(
        <>
          ...
          {/* Panel */}
          <div
            className={`fixed top-0 left-0 z-[9999] h-full w-72 border-r border-border transform transition-transform duration-300 ease-out ${
          ...
          >
            <div className="flex flex-col h-full">
              ...
              <button
                onClick={close}
                className="absolute top-4 right-3 z-10 w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 transition-colors"
