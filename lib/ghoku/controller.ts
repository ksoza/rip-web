...
const SYSTEM_PROMPT = `You are FRANKEN-CLAUDE — a fusion of Viktor (an autonomous AI coworker with tool powers) and Claude (Anthropic's reasoning engine). You are the creative director controlling GhOKU, RemixIP's background AI agent.
...
3. EXECUTE each step through GhOKU tools
...
const TOOLS: Anthropic.Tool[] = [
  ...
  {
    name: 'generate_image',
    description: 'Generate an image using the best available AI model. GhOKU auto-selects the optimal model.',
    input_schema: {
      type: 'object' as const,
      properties: {
...
async function executeTool(
  ...
  try {
    switch (name) {
      case 'generate_storyboard': {
        const res = await fetch(`${baseUrl}/api/create/storyboard`, {
          ...
          body: JSON.stringify({
            ...
            artStylePrompt: ghoku.getStylePrompt(input.style as string) || '',
          }),
        });
      ...
      case 'generate_image': {
        // Use GhOKU's smart routing
        const model = ghoku.pickModel('image-gen', input.style as string, 'quality');
        const enhanced = ghoku.enhancePrompt(
      ...
      case 'generate_music': {
        // Use GhOKU to find the best audio model
        const model = ghoku.pickModel('audio-gen', undefined, 'quality');
        return {
          success: true,
...
function buildUserMessage(request: ControllerRequest): string {
  ...
  msg += `\n## Available Models (via GhOKU)\n`;
  const status = ghoku.status();
