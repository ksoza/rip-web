...
export async function discoverModels(
  ...
  for (const tag of pipelineTags) {
    try {
      ...
      if (!res.ok) {
        console.warn(`[ghostface/discovery] HF API error for ${tag}: ${res.status}`);
        continue;
      }
      ...
      console.warn(`[ghostface/discovery] Failed to scan ${tag}:`, err);
    }
  }

  console.log(
    `[ghostface/discovery] Found ${discovered.length} new ${category} models`
  );
  ...
  for (const cat of cats) {
    ...
    for (const model of models) {
      ...
      if (!registry.get(model.id)) {
        ...
        totalRegistered++;
        console.log(`[ghostface/discovery] Registered new model: ${model.id}`);
      }
    }
  }
...
export async function testModel(
  ...
  try {
    ...
    console.log(
      `[ghostface/discovery] Test ${model.endpoint}: ${res.status} → ${works ? 'OK' : 'FAIL'}`
    );
