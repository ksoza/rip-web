...
export async function executeWithFallback(
  ...
  while (attempts < MAX_RETRIES) {
    ...
    try {
      console.log(
        `[ghostface/fallback] Attempt ${attempts + 1}/${MAX_RETRIES}: ` +
      ...
      console.warn(
        `[ghostface/fallback] ${model.id} failed (attempt ${attempts}): ${error}`
      );
      ...
      if (isTransient && attempts < MAX_RETRIES) {
        ...
        console.log(`[ghostface/fallback] Transient error, retrying in ${delay}ms...`);
        await sleep(delay);
      } else {
        // Permanent failure — exclude this model and try next
        excludeModels.push(model.id);
        console.log(`[ghostface/fallback] Excluding ${model.id}, trying next model...`);
      }
    }
  }
