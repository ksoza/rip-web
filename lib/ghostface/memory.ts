...
export function getCached(prompt: string, style?: string): { url: string; modelId: string } | null {
  ...
  if (cached && Date.now() - cached.timestamp < PROMPT_CACHE_TTL) {
    console.log(`[ghostface/memory] Cache hit for prompt: ${prompt.slice(0, 50)}...`);
...
async function persistToSupabase(entry: {
  ...
  try {
    await fetch(`${supabaseUrl}/rest/v1/ghostface_memory`, {
      method: 'POST',
      headers: {
...
export async function loadFromSupabase(): Promise<void> {
  ...
  try {
    const res = await fetch(
      `${supabaseUrl}/rest/v1/ghostface_memory?select=model_id,success,latency_ms&order=created_at.desc&limit=500`,
      {
        headers: {
          apikey: supabaseKey,
          Authorization: `Bearer ${supabaseKey}`,
    ...
    console.log(`[ghostface/memory] Loaded ${rows.length} history entries from Supabase`);
  } catch {
    // Silent fail
  }
}
